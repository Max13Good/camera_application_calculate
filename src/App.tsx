...
export default function App() {
  ...
  const unitTariffRows: UnitTariffRow[] = useMemo(() => {
    ...
    for (const t of tariffs) {
      ...
      const yandexCostAcc =
        storageGBacc * storagePrice + cdnGBacc * cdnPrice + backendOpsPerAcc;
  ...
  const tariffPortfolio = useMemo(() => {
    ...
    const rows = tariffs.map((t) => {
      ...
      const sumBase =
        ts.reduce((s, x) => s + (x.forecast?.baseShare0 || 0), 0) || 1;
      ...
      for (const t of tariffs) {
        ...
        const tuyaCostPerAcc =
          Math.max(0, (apiCallsPerAcc - 1_000_000) / 1_000_000) *
            vcApiUSD *
            fx +
          (msgsPerAcc / 1_000_000) * vcMsgUSD * fx +
          relayGBPerAcc * vcRelayUSD * fx;
