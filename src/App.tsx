...
function BaseMixEditor({ tariffs, setTariffs }: { tariffs: any[]; setTariffs: (t: any[]) => void }) {
  const families: Array<{ key: "camera" | "smarthome" | "bundle"; title: string }> = [
    { key: "camera", title: "Камеры" },
    { key: "smarthome", title: "Умный дом" },
    { key: "bundle", title: "Комбо" },
  ...
  const updateShare = (id: string, nextVal: number) => {
    ...
    setTariffs(tariffs.map((t:any) => t.id === id ? { ...t, forecast: { ...(t.forecast||{}), baseShare0: v } } : t));
  };

  const normalizeFamily = (fam: "camera"|"smarthome"|"bundle") => {
    const list = tariffs.filter((t:any) => t.family === fam);
    const sum = list.reduce((s:number,t:any)=> s + (t.forecast?.baseShare0 || 0), 0);
    if (sum <= 0) return;
    setTariffs(tariffs.map((t:any)=> t.family===fam ? { ...t, forecast:{ ...(t.forecast||{}), baseShare0: (t.forecast?.baseShare0 || 0)/sum } } : t));
  };

  const evenFamily = (fam: "camera"|"smarthome"|"bundle") => {
    const list = tariffs.filter((t:any) => t.family === fam);
    const w = list.length ? 1 / list.length : 0;
    setTariffs(tariffs.map((t:any)=> t.family===fam ? { ...t, forecast:{ ...(t.forecast||{}), baseShare0: w } } : t));
  };
        ...
        return (
          <div key={f.key} className="bg-gray-50 rounded-xl p-4 border border-gray-100">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div className="text-xs text-gray-600">Сумма долей: <span className={`font-medium ${Math.abs(sum-1)<1e-6?"text-green-600":"text-amber-600"}`}>{new Intl.NumberFormat('ru-RU',{maximumFractionDigits:2}).format(sum)}</span></div>
              <div className="ml-auto flex gap-2 text-sm">
                <button className="px-2 py-1 rounded-lg bg-gray-100 hover:bg-gray-200" onClick={()=>normalizeFamily(f.key)}>Нормализовать</button>
                <button className="px-2 py-1 rounded-lg bg-gray-100 hover:bg-gray-200" onClick={()=>evenFamily(f.key)}>Равномерно</button>
              </div>
            </div>
              ...
              <table className="min-w-[700px] table-auto text-sm">
                <thead>
                  <tr className="text-left">
                    ...
                    <th className={thCls}>Доля в базе (0..1) <Help text="Распределение текущей базы внутри семейства. Сумма по семейству = 1." /></th>
                  </tr>
                </thead>
                <tbody>
                  {list.map((t:any)=> (
                    <tr key={t.id} className="border-t">
                      ...
                      <td className={tdTextCls}>
                        <input className={`${inputCls} w-32`} type="number" step={0.01} value={t.forecast?.baseShare0 ?? 0} onChange={(e)=>updateShare(t.id, Number(e.target.value))} />
                      </td>
                    </tr>
                  ))}
                </tbody>
...
export default function App() {
  ...
  const unitTariffRows: UnitTariffRow[] = useMemo(() => {
    ...
    for (const t of tariffs) {
      ...
      const apiPerAcc = baseApiPerAcc * (apiPriceUSDpm / Math.max(tuyaApiUSDpm, 1e-9)) * (vc.tuyaApiMul ?? 1);
      const msgsPerAcc = baseMsgsPerAcc * (msgsPriceUSDpm / Math.max(tuyaMsgUSDpm, 1e-9)) * (vc.tuyaMsgsMul ?? 1);
      const relayPerAcc = baseRelayPerAcc * (relayPriceUSDpGB / Math.max(tuyaRelayUSDpGB, 1e-9)) * (vc.tuyaRelayMul ?? 1);
        ...
        {tab === "calc" && (
          <>
            ...
            <Section title="Распределение текущей базы (микс внутри семейств)">
              <div className="text-sm text-gray-700 mb-3">
                Настройте доли (0..1) по тарифам для каждой продуктовой линии. Сумма по семейству должна быть равна 1. Кнопки нормализуют/распределяют равномерно.
              </div>
