...
export default function App() {
  ...
  const forecast = useMemo(() => {
    ...
    const mixMap: Record<string, number> = Object.fromEntries(
      pkgs.map((p) => [p.name, p.mix])
    );
    const w = (key: keyof UnitRow): number =>
      unitRows.reduce(
        (s, r) => s + (r[key] as number) * (mixMap[r.name] ?? 0),
        0
      );
  ...
  const breakevenSim = useMemo(() => {
    ...
    for (let m = 1; m <= 60; m++) {
      ...
      const revenue = activeCloudAcc * revPerAcc;
      const cost =
        activeCloudAcc * (yCostPerAcc + tCostPerAcc) +
        backendFixed +
        opsFixed +
        tuyaSDKmoRUB;
    ...
    const firstPositive = rows.find((r) => r.cumProfit > 0)?.month ?? null;
    return { rows, firstPositive };
  }, [
    avgCams,
    backendFixed,
    opsFixed,
  ...
  // ---------------- UI ----------------
  const [tab, setTab] = useState<"calc" | "forecast" | "breakeven">("calc");
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900">
      <div className="max-w-7xl mx-auto p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-700">
            ...
            <div>
              ...
              <ul className="list-disc pl-5 space-y-1">
                ...
                <li>
                  Отредактируйте <b>пакеты</b>: дни архива, лимит ГБ/кам,
                  включённые камеры, цены, стоимость перерасхода и долю в миксе.
                </li>
                <li>
                  Смотрите расчёт <b>на аккаунт</b> и{" "}
                  <b>суммарно по портфелю</b>.
                </li>
                <li>
                  Во вкладке «Прогноз» укажите: горизонт (мес), churn, продажи в
                      ...
                      <tr key={p.name} className="border-t">
                        ...
                        <td className={tdTextCls}>
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              next[idx] = {
                                ...p,
                                capPerCamGB: Number(e.target.value),
                              };
                              setPkgs(next);
                            }}
                          />
                        ...
                        <td className={tdTextCls}>
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              next[idx] = {
                                ...p,
                                addlCamPrice: Number(e.target.value),
                              };
                              setPkgs(next);
                            }}
                          />
                        ...
                        <td className={tdTextCls}>
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              next[idx] = {
                                ...p,
                                overagePerGB: Number(e.target.value),
                              };
                              setPkgs(next);
                            }}
                          />
                        ...
                        <td className={tdTextCls}>
                          <input
                            ...
                            onChange={(e) => {
                              const next = [...pkgs];
                              next[idx] = { ...p, mix: Number(e.target.value) };
                              setPkgs(next);
                            }}
                          />
                        </td>
                ...
                <table className="min-w-[1100px] table-auto text-sm">
                  <thead>
                    <tr className="text-left">
                      ...
                      <th className={thCls}>
                        Выручка/акк{" "}
                        <Help text="Цена пакета + оплата за доп. камеры сверх включённых." />
                      </th>
                      <th className={thCls}>
                        Overage/акк{" "}
                        <Help text="Плата за перерасход лимита ГБ (факт − лимит, если > 0)." />
                      </th>
                      <th className={thCls}>
                        Yandex cost/акк{" "}
                        <Help text="Хранилище + CDN + доля фиксированных затрат." />
                      </th>
                      <th className={thCls}>
                        Tuya cost/акк{" "}
                        <Help text="SDK + API + Msgs + Relay, распределённые на аккаунт." />
                      </th>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <Section title="Ключевые показатели">
                <div className="grid grid-cols-2 gap-2 text-sm">
                  ...
                  <div className="font-semibold">
                    <Num value={last?.activeCloudAcc ?? 0} />
                  </div>
                </div>
              </Section>
        ...
        {tab === "breakeven" && (
          <>
            <Section title="Точка безубыточности — модель с нуля">
              <div className="text-sm mb-3">
                Модель начинается без аккаунтов. Считаются только фиксированные
                затраты (Tuya SDK, backend, ops). Продажи нарастают по заданным
                параметрам. Таблица показывает, когда накопительная прибыль
                становится больше 0.
              </div>
              <div className="mb-4">
                <span className="font-semibold">Месяц выхода в плюс: </span>
                {breakevenSim.firstPositive
                  ? `Месяц ${breakevenSim.firstPositive}`
                  : "не достигнут в горизонте"}
              </div>
              <div className="overflow-x-auto">
                <table className="min-w-[1100px] table-auto text-xs md:text-sm">
                  <thead>
                    <tr className="text-left">
                      <th className={thCls}>Мес</th>
                      <th className={thCls}>Продажи камер</th>
                      <th className={thCls}>Новые акк</th>
                      <th className={thCls}>Новые облачные</th>
                      <th className={thCls}>Активные облачные</th>
                      <th className={thCls}>Выручка</th>
                      <th className={thCls}>Затраты (фикс+перем)</th>
                      <th className={thCls}>Прибыль</th>
                      <th className={thCls}>Накопит. прибыль</th>
                    </tr>
                  </thead>
                  <tbody>
                    {breakevenSim.rows.map((r) => (
                      <tr key={r.month} className="border-t">
                        <td className={tdTextCls}>{r.month}</td>
                        <td className={tdNumCls}>
                          <Num value={r.sales} />
                        </td>
                        <td className={tdNumCls}>
        ...
        <Section title="Примечания и допущения">
          <ul className="list-disc pl-5 text-sm text-gray-700 space-y-1">
            ...
            <li>
              ...
              общей базе камер; уточняйте тарифы у Tuya — это ориентиры.
            </li>
            <li>
